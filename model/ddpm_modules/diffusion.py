...
class GaussianDiffusion(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.channels = 2 # For thermal + noise
        self.image_size = image_size
        # self.denoise_fn = denoise_fn
        self.denoise_fn = MyDenoiseModel(in_channels=2, out_channels=1)
    ...
    def set_new_noise_schedule(self, schedule_opt, device):
        ...
        self.register_buffer('betas', to_torch(betas.reshape(-1, 1, 1)))
    ...
    def predict_start_from_noise(self, x_t, t, noise):
        return (
            extract(self.sqrt_recip_alphas_cumprod, t, x_t.shape).unsqueeze(-1).unsqueeze(-1) * x_t -
            extract(self.sqrt_recipm1_alphas_cumprod, t, x_t.shape).unsqueeze(-1).unsqueeze(-1) * noise
        )
    ...
    def p_sample_loop(self, x_in, mask, continous=False):
        ...
        if not self.conditional:
            shape = (x_in.shape[0], 1, x_in.shape[2], x_in.shape[3])
            b = shape[0]
    ...
    def sample(self, batch_size=1, continous=False):
        ...
        return self.p_sample_loop(torch.randn(batch_size, 1, self.image_size, self.image_size).to(self.betas.device)
)

    @torch.no_grad()
    def super_resolution(self, x_in, mask, continous=False):
        return self.p_sample_loop(x_in, mask, continous).view(x_in.shape[0], 1, x_in.shape[2], x_in.shape[3])
    ...
    def q_sample(self, x_start, t, noise=None):
        ...
        continuous_sqrt_alpha_cumprod = extract(self.sqrt_alphas_cumprod, t, x_start.shape).unsqueeze(-1).unsqueeze(-1)


        return continuous_sqrt_alpha_cumprod * x_start + extract(self.sqrt_one_minus_alphas_cumprod, t, x_start.shape).unsqueeze(-1).unsqueeze(-1) * noise
    ...
    def p_losses(self, x_in, noise=None):
        ...
        if not self.conditional:
            ...
            x_recon = self.denoise_fn(
                torch.cat([x_in['SR'], x_in['mask'], x_noisy], dim=1), t)
