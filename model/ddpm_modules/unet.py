...
class TimeEmbedding(nn.Module):
    ...
    def forward(self, input):
        ...
        pos_emb = pos_emb.view(shape[0], self.dim)
        return pos_emb
...
class SelfAttention(nn.Module):
    ...
    def forward(self, input):
        ...
        attn = torch.einsum(
            "bnchw, bncxy -> bnhwxy", query, key
        ) / math.sqrt(head_dim)

        attn = attn.view(batch, n_head, height, width, -1)
        attn = torch.softmax(attn, dim=-1) + 1e-8
...
class UNet(nn.Module):
    def __init__(
        self,
        in_channel = 2,  # For thermal + noise
        out_channel = 1,  # For single-channel depth map output
        inner_channel=32,
        norm_groups=32,
