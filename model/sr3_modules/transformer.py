...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, xm, attn_kv=None, mask=None):
        ...
        # mm = torch.bmm(xm, xm.permute(0, 2, 1))
...
class LeWinTransformerBlock1(nn.Module):
    ...
    def forward(self, x, xm, mask=None, img_size=(128, 128)):
        ...
        # attn_windows1 = torch.mean(attn_windows1, dim=2)
        # hh = self.win_size[0]
        # ww = self.win_size[1]
...
class Uformer_sub(nn.Module):
    def __init__(self, img_size=240, in_chans=3,
        ...
        self.decoderlayer_1 = BasicUformerLayer(dim=embed_dim*12,
                            output_dim=embed_dim*12,
                            input_resolution=(img_size // (2 ** 2),
                                                img_size // (2 ** 2)),
                            depth=depths[6],
        ...
        self.decoderlayer_2 = BasicUformerLayer(dim=embed_dim*6,
        ...
        self.decoderlayer_3 = BasicUformerLayer(dim=embed_dim*3,
        ...
        self.gated_conv = nn.Conv2d(4, 3, 3, 1, 1)
        self.gated_conv1 = nn.Conv2d(4, 3, 3, 1, 1)
    ...
    def forward(self, x, pre_x, xm, pre_conv0, pre_conv1, pre_conv2, mask=None):
        # Input  Projection
        xi = torch.cat((x, pre_x, xm), dim=1)
        # xii = torch.cat((x, xcon), dim=1)
        # coni = self.gated_conv1(xii)
        self.img_size = (x.shape[2], x.shape[3])
        y = self.input_proj(xi)
        # cony = self.input_proj(coni)
        y = self.pos_drop(y)
        #Encoder
        conv0 = self.encoderlayer_0(y, mask=mask, img_size = self.img_size)
        ...
        self.img_size = (int(self.img_size[0] / 2), int(self.img_size[1] / 2))
        # conv3 = self.encoderlayer_3(pool2, mask=mask, img_size = self.img_size)
        # pool3 = self.dowsample_3(conv3, img_size = self.img_size)
        # self.img_size = (int(self.img_size[0] / 2), int(self.img_size[1] / 2))

        # Bottleneck
        conv4 = self.conv(pool2, mask=mask, img_size = self.img_size)

        #Decoder
        # up0 = self.upsample_0(conv4, img_size = self.img_size)
        # self.img_size = (int(self.img_size[0] * 2), int(self.img_size[1] * 2))
        # deconv0 = torch.cat([up0,conv3],-1)
        # deconv0 = self.decoderlayer_0(deconv0, mask=mask, img_size = self.img_size)

        up1 = self.upsample_1(conv4, img_size = self.img_size)
        self.img_size = (int(self.img_size[0] * 2), int(self.img_size[1] * 2))
        deconv1 = torch.cat([up1,conv2, pre_conv2],-1)
        deconv1 = self.decoderlayer_1(deconv1, mask=mask, img_size = self.img_size)

        up2 = self.upsample_2(deconv1, img_size = self.img_size)
        self.img_size = (int(self.img_size[0] * 2), int(self.img_size[1] * 2))
        deconv2 = torch.cat([up2,conv1, pre_conv1],-1)
        deconv2 = self.decoderlayer_2(deconv2, mask=mask, img_size = self.img_size)

        up3 = self.upsample_3(deconv2, img_size = self.img_size)
        self.img_size = (int(self.img_size[0] * 2), int(self.img_size[1] * 2))
        deconv3 = torch.cat([up3,conv0, pre_conv0],-1)
        deconv3 = self.decoderlayer_3(deconv3, mask=mask, img_size = self.img_size)

        # Output Projection
        y = self.output_proj(deconv3, img_size = self.img_size) + x
        return y, conv0, conv1, conv2
...
class Uformer_sub1(nn.Module):
    ...
    def forward(self, x, xm, mask=None):
        ...
        m = nn.MaxPool2d(2)
        xm2 = m(xm1)
        # xm22 = xm2.view(xm2.shape[0], xm2.shape[2] * xm2.shape[3], 1)
        self.img_size = (int(self.img_size[0] / 2), int(self.img_size[1] / 2))
        ...
        m = nn.MaxPool2d(2)
        xm3 = m(xm2)
        # xm33 = xm3.view(xm3.shape[0], xm3.shape[2] * xm3.shape[3], 1)
        # Bottleneck
        conv4 = self.conv(pool2, xm3, mask=mask, img_size = self.img_size)
        ...
        deconv1 = self.decoderlayer_1(deconv1, xm2, mask=mask, img_size = self.img_size)

        up2 = self.upsample_2(deconv1, img_size = self.img_size)
        self.img_size = (int(self.img_size[0] * 2), int(self.img_size[1] * 2))
        deconv2 = torch.cat([up2,conv1],-1)
        deconv2 = self.decoderlayer_2(deconv2, xm1, mask=mask, img_size = self.img_size)

        up3 = self.upsample_3(deconv2, img_size = self.img_size)
        self.img_size = (int(self.img_size[0] * 2), int(self.img_size[1] * 2))
        deconv3 = torch.cat([up3,conv0],-1)
        deconv3 = self.decoderlayer_3(deconv3, xm, mask=mask, img_size = self.img_size)
...
class Uformer(nn.Module):
    ...
    def forward(self, x_input, mask):
        ...
        # output2, conv02, conv12, conv22 = self.recur2(output1, x_input, mask, conv01, conv11, conv21)
        # output3, conv03, conv13, conv23 = self.recur3(output2, x_input, mask, conv02, conv12, conv22)
        return output
...
class Uformer_Cross(nn.Module):
    def __init__(self, img_size=128, in_chans=4,
